...
| 条件 | 抑制方向 | 保留方 |
| --- | --- | --- |
| 非首行的 td/th（trParent 中存在更早的 TR） | `borderTop` → `0px / none` | 上一行的 `borderBottom` |
| 非首列的 td/th（TR 中存在更早的 TD/TH） | `borderLeft` → `0px / none` | 左侧格子的 `borderRight` |

首行/首列的格子不受影响，保留表格外边缘的边框。

### 相关函数调用链

```
parseElement()
  └── resolveCellBorderOverrides(tag, isPseudo, measEl, win)
        ├── shouldSuppressCellBorderTop(cellEl, win)
        │     └── getCollapseTable(cellEl, win)
        └── shouldSuppressCellBorderLeft(cellEl, win)
              └── getCollapseTable(cellEl, win)
```

`resolveCellBorderOverrides` 返回 `{ top, left } | null`，每个方向为覆盖对象 `{ width, color, style }` 或 `null`（不抑制）。
...
// 元素节点
{
  ...
  style: {
    ...
    // 边框（border-collapse 表格的 td/th：borderTop/borderLeft 可能已被抑制为 0px/none）
